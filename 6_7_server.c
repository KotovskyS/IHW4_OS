...
int main(int argc, char *argv[]) {
    ...
    int server_socket, broadcast_socket;
    ...
    setsockopt(broadcast_socket, SOL_SOCKET, SO_BROADCAST, &broadcastEnable, sizeof(broadcastEnable));
    memset(&broadcast_addr, 0, sizeof(broadcast_addr));
    broadcast_addr.sin_family = AF_INET;
    inet_pton(AF_INET, "255.255.255.255", &broadcast_addr.sin_addr);
    broadcast_addr.sin_port = htons(40000);  // Порт для широковещательных сообщений
    ...
    for (int i = 0; i < num_clients; i++) {
        ...
        recvfrom(server_socket, encrypted_text + fragment_start, fragment_size, 0, (struct sockaddr *) &client_addr,
                 &client_addr_len);
        char message[50];
        sprintf(message, "Клиент %d обработал часть сообщения", i + 1);
        sendto(broadcast_socket, message, strlen(message), 0, (struct sockaddr *) &broadcast_addr,
